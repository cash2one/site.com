...
class storeModel extends Model {
    ...
    public function getStoreListByGoodsClassId_1_5_4($gc_id){
        ...
        $store_list = Model()->query("SELECT DISTINCT S.store_id,S.store_name,S.store_label FROM wantease_goods AS G LEFT JOIN wantease_store AS S ON G.store_id = S.store_id WHERE (G.gc_id_1=$gc_id OR G.gc_id_2=$gc_id OR G.gc_id_3=$gc_id) AND S.store_state=1 ORDER BY S.store_collect DESC LIMIT 9");
       /*
        $model = Model();
        $on = 'goods.store_id = store.store_id';
        $where = array();
        $where = "(goods.gc_id_1 = $gc_id) || (goods.gc_id_2 = $gc_id) || (goods.gc_id_3 = $gc_id)";
       // $where = "goods.gc_id_1 = $gc_id";
        //$where[1] = "store.store_state = 1";
        $field = 'store.store_id,store.store_name,store.store_label';
       // $order = 'store.store_collect DESC';
        $model->table('goods,store')->field($field)->distinct('true');
        $store_list = $model->join('left')->on($on)->where($where)->limit(9)->select();
     */
        return $store_list;
