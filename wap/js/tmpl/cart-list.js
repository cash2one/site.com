$(function () {
    ...
    if (!t) {
        ...
        $(".goods-del").click(function () {
            var t = $(this);
            $.sDialog({
                skin: "red", content: "确认删除吗？", okBtn: true, cancelBtn: true, okFn: function () {
        ...
        function _(a, e) {
            ...
            $.ajax({
                ...
                type: "post",
                data: {key: t, cart_id: o, quantity: s},
                dataType: "json",
                success: function (t) {
                    if (checkLogin(t.login)) {
                        if (!t.datas.error) {
                            ...
                            $.sDialog({skin: "red", content: t.datas.error, okBtn: false, cancelBtn: false})
                        }
        ...
        $.sValid.init({
            rules: {buynum: "digits"}, messages: {buynum: "请输入正确的数字"}, callback: function (t, a, e) {
                if (t.length > 0) {
                    var r = "";
                    $.map(a, function (t, a) {
                        r += "<p>" + t + "</p>"
