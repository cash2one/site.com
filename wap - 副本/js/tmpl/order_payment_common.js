...
var password, rcb_pay, pd_pay, onempf_pay,payment_code;
        ...
        success: function(p) {
            ...
            if (parseFloat(p.datas.pay_info.payed_amount) <= 0) {
                if (parseFloat(p.datas.pay_info.member_available_pd) == 0 && parseFloat(p.datas.pay_info.member_available_rcb) == 0) {
                    ...
                    if (parseFloat(p.datas.pay_info.member_available_onempf) != 0) {
                        $("#wrapperUseONEpy").show();
                        $("#availableOnempf").html(parseFloat(p.datas.pay_info.member_available_onempf).toFixed(2))
                    } else {
                        $("#wrapperUseONEpy").hide()
                    }
                }
            } else {
                $("#internalPay").hide()
...
function goToPayment(a, e) {
	location.href = ApiUrl + "/index.php?act=member_payment&op=" + e + "&key=" + key + "&pay_sn=" + a + "&password=" + password + "&rcb_pay=" + rcb_pay + "&pd_pay=" + pd_pay + "&onempf_pay=" + onempf_pay + "&payment_code=" + payment_code
}