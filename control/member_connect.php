...
class member_connectControl extends BaseMemberControl {
	public function __construct() {
		 ...
		 * 读取语言包
		 */
		Language::read('member_member_qqconnect,member_member_sconnect,member_member_wxconnect');
	}
	/**
	 * QQ绑定
	 */
    ...
    public function wxbindOp(){
        //获得用户信息
        if (trim($this->member_info['wx_info'])){
            $this->member_info['wx_infoarr'] = unserialize($this->member_info['wx_info']);
        }
        Tpl::output('member_info',$this->member_info);
        //信息输出
    ...
    public function wxunbindOp(){
        ...
        if ($_POST['is_editpw'] == 'yes'){
            ...
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
                array("input"=>$_POST["new_password"],      "require"=>"true","validator"=>"Length","min"=>6,"max"=>20,"message"=>Language::get('member_wxconnect_password_null')),
                array("input"=>$_POST["confirm_password"],  "require"=>"true","validator"=>"Compare","operator"=>"==","to"=>$_POST["new_password"],"message"=>Language::get('member_wxconnect_input_two_password_again')),
            );
            $error = $obj_validate->validate();
            if ($error != ''){
                showMessage($error,'','html','error');
        ...
        session_unset();
        session_destroy();
        showMessage('微信解绑成功','index.php?act=login&ref_url='.urlencode('index.php?act=member_connect&op=wxbind'));
    }
	/**
	 * 用户中心右边，小导航
	 *
	...
	private function profile_menu($menu_key='',$array=array()) {
		...
		$menu_array = array(
			...
			3=>array('menu_key'=>'wx_bind',	'menu_name'=>$lang['nc_member_path_wx_bind'],	'menu_url'=>'index.php?act=member_connect&op=wxbind'),
		);
		if(!empty($array)) {
			$menu_array[] = $array;
		}
