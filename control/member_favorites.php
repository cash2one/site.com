...
class member_favoritesControl extends BaseMemberControl{
	 ...
	 * @param
	 * @return
	 * update by lizh 11:53 2016/6/30
	 */
	public function fglistOp(){
         ...
         * 商品收藏列表
         */
        $favorites_model_class = Model('favorites_class');
        $favorites_class_list = $favorites_model_class -> where(array('member_id'=>$_SESSION['member_id'],favorites_class_type => 'goods')) -> select();
        Tpl::output('favorites_class_list',$favorites_class_list);
        $favorites_class_id = $_GET['favorites_class_id'];
        if(empty($favorites_class_id)) {

                $favorites_class_id = 0;

        }
		
		$favorites_list = $favorites_model->getGoodsFavoritesList(array('member_id'=>$_SESSION['member_id'], 'favorites_class_id' => $favorites_class_id), '*', 20);
	 ...
	 * @return
	 *
	 * update by lizh 10:54 2016/6/30
	 */
