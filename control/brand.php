...
class brandControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        //获得品牌列表
        $model = Model();
        $brand_c_list = $model->table('brand')->where(array('brand_apply' => '1'))->order('brand_initial asc')->select();
        $brands = $this->_tidyBrand($brand_c_list);
        extract($brands);
           $model_goods = Model('goods');
        if (!empty($brand_r_list)) {
            foreach ($brand_r_list as $key => $val) {
                $condition['brand_id'] = $brand_r_list[$key]['brand_id'];
//                $condition['goods_state'] = 1;
                $brand_r_list[$key]['goods_list'] = $model_goods->getGoodsList($condition, $field, 'rand()', $limit = 0, 5);
            }
        }
       
        //var_dump($brand_r_list);exit;
        Tpl::output('brand_c', $brand_listnew);
        Tpl::output('brand_country', $brand_country);
    ...
    public function listOp() {
        ...
        Tpl::output('show_page1', $model_goods->showpage(5));
