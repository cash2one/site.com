...
class show_groupbuyControl extends BaseHomeControl {
    ...
    public function indexOp(){
        ...
        // 限量抢购
        $condition['is_vr'] = 0;
        $condition['groupbuy_type'] = 1;
        $groupbuy_list_num = $model_groupbuy->getGroupbuyExtendList($condition);

       // var_dump($groupbuy_list_num);

        if (!empty($groupbuy_list_num)) {
            foreach ($groupbuy_list_num as $key => $val) {
                if ($val['end_time'] < TIMESTAMP) {
                    unset($groupbuy_list_num[$key]);//删除已结束抢购
                }
            }
        }
        //删除后，下标重排序
        $groupbuy_list_num = array_merge($groupbuy_list_num);
        Tpl::output('groupbuy_list_num', $groupbuy_list_num);

        $goodslist = Model('goods_browse')->getViewedGoodsList($_SESSION['member_id'], 4);
        Tpl::output('guesslike',$goodslist);
