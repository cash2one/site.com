  ...
  <form id="news_form" method="post" name="newsForm">
    ...
    <table class="table tb-type2 nobdb">
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><select name="nc_id" id="nc_id">
              <option value="">请选择...</option>
              <?php if(!empty($output['parent_list']) && is_array($output['parent_list'])){ ?>
              <?php foreach($output['parent_list'] as $k => $v){ ?>
              <option <?php if($output['nc_id'] == $v['nc_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['nc_id'];?>"><?php echo $v['nc_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
          <td class="vatop tips"></td>
        ...
        <tr>
          <td colspan="2" class="required"><label for="newsForm">文章链接:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="" name="news_url" id="news_url" class="txt"></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="" name="vedio_url" id="vedio_url" class="txt"></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform onoff"><label for="news_show1" class="cb-enable selected" ><span>是</span></label>
            <label for="news_show0" class="cb-disable" ><span>否</span></label>
            <input id="news_show1" name="news_show" checked="checked" value="1" type="radio">
            <input id="news_show0" name="news_show" value="0" type="radio"></td>
          <td class="vatop tips"></td>
        </tr>
        <tr>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="255" name="news_sort" id="news_sort" class="txt"></td>
...
$(document).ready(function(){
    ...
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?act=news&op=news_pic_upload',
