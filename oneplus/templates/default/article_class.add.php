  ...
  <form id="article_class_form" method="post" enctype="multipart/form-data">
    ...
    <table class="table tb-type2">
      <tbody>
		...
		<tr>
          <td colspan="2" class="required"><label for="parent_id">分类类型:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
			<input type="radio" value="pc" id="ac_type_pc" name="ac_type" checked="checked"><label for="ac_type_pc">PC文章</label>
			<input type="radio" value="wap" id="ac_type_wap" name="ac_type"><label for="ac_type_wap">WAP文章</label>
		  </td>
          <td class="vatop tips">*默认为PC端的文章</td>
          
        </tr>
		<!-- <tr class="noborder" id="upload_toggle" style="display:none">
          <td class="vatop rowform">
			<input type="file" id="ac_icon" name="ac_icon">
		  </td>
          <td class="vatop tips">*WAP端显示的图片</td>
...
$(function(){
	...
	$("*[name='ac_type']").click(function(){
		
		var chkObjs = document.getElementsByName("ac_type");
		for(var i=0;i<chkObjs.length;i++){
			if(chkObjs[i].checked){
				if(chkObjs[i].value == 'pc') {
					
					//$("#upload_toggle").css('display','none');
					
				} else {
					
					//$("#upload_toggle").css('display','table-row');
