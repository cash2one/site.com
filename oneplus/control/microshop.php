...
//	}
    ...
    public function personal_manageOp() {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($class_id == 0) {
                ...
                $class_name = $micro_personal_class->getfby_class_id($class_id, 'class_name');
            }

            $list[$k]['class_name'] = $class_name;
            $list[$k]['class_id'] = $class_id;
        }

        $class_list = $micro_personal_class->select();
    ...
    public function personal_dropOp() {
        ...
        $condition['fav_id'] = array('in',trim($_POST['personal_id']));
        $rs = $favorites->delFavorites($condition);
        
        $micro_comment = Model('micro_comment');
        $condition = array();
        $condition['comment_object_id'] = array('in',trim($_POST['personal_id']));
     ...
     * 更新微商城个人秀排序
     */
    public function personal_like_count_updateOp() {
        $this->update_like_count('micro_personal', 'personal_id');
    }

    private function update_like_count($model_name, $key_name) {
        if (intval($_GET['id']) <= 0) {
            echo json_encode(array('result' => FALSE, 'message' => Language::get('param_error')));
            die;
        }
        $new_sort = intval($_GET['value']);
        $model_class = Model($model_name);
        $result = $model_class->modify(array('like_count' => $new_sort), array($key_name => $_GET['id']));
	...
	/**
	 * 设置瞬间的话题
	 * add by lizh 15:38 2016/07/20
	 */
	public function setclass_listOp() {

        $class_id = $_GET['class_id'];
        $personal_id = $_GET['personal_id'];

        $micro_personal = Model('micro_personal');
        $rs = $micro_personal->modify(array(class_id => $class_id), array(personal_id => $personal_id));
    ...
    /*
     * 随机增加10个人的点赞数量
     * add by niro 2016/9/10
     */

    public function micro_like_addOp() {
        $model_like = Model('micro_like');
        $model_micro_personal = Model('micro_personal');
        $list = $model_micro_personal->getList(array(), 10, 'like_count asc', '*');
...
//        if (!$is_exist) {
            $temp = array_rand($param, 10);
            foreach ($temp as $value) {
                $paramArr[] = $param[$value];
            }
            $result = $model_like->saveAll($paramArr);
            if ($result) {
                $model = Model();
                //喜欢计数加1
                $update = array(
                    'like_count' => array('exp', 'like_count+10')
                );
                $condition = array();
                $condition['personal_id'] = $personal_id;
                $model->table("micro_personal")->where($condition)->update($update);
                $model_micro_personal->getOne($condition, '', 'like_count');
...
//                }
    ...
    public function mirco_likeOp() {
        $menu_array = array(
        );
        Tpl::output('menu', $menu_array);
        Tpl::showpage('mircoshop_like.add');
    }
