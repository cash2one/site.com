	...
	public function article_class_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				//update by lizh 15:06 2016/7/7
				$upload = new UploadFile();
				$upload->set('default_dir',ATTACH_CMS.'/ac_icon');
				if(!empty($_FILES['ac_icon'])) {
					
					$result	= $upload->upfile('ac_icon');
					$insert_array['ac_icon'] = $upload->file_name;
					
				}
				$insert_array['ac_type'] = trim($_POST['ac_type']);
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			...
			case 'check_class_name':
				...
				$condition['ac_type'] = trim($_GET['ac_type']);
