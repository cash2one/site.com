...
class uploadControl extends SystemControl{
	...
	public function serviceOp(){
		if (chksubmit()){
			$obj_validate = new Validate();
			$obj_validate->validateparam = array(
				array("input"=>$_POST["upload_servic_type"], "require"=>"true", "message"=>L('upload_servic_type_is_null')),
				array("input"=>$_POST["upload_servic_host"], "require"=>"true", "message"=>L('upload_servic_host_is_null')),
				array("input"=>trim($_POST["upload_servic_username"]), "require"=>"true", "message"=>L('upload_servic_username_is_null')),
				array("input"=>trim($_POST["upload_servic_password"]), "require"=>"true", "message"=>L('upload_servic_password_is_null')),
				array("input"=>trim($_POST["upload_servic_domain"]), "require"=>"true", "message"=>L('upload_servic_domain_is_null'))
			);
			$error = $obj_validate->validate();
			if ($error != ''){
				showMessage($error);
