			...
			if ($error != ''){
				showMessage($error);
                            ...
                            if (!empty($_FILES['brand_banner']['name'])){
                                    $result = $upload->upfile('brand_banner');
                                    if (!$result){
                                            showMessage($upload->error,'','','error');
                                    }else{
                                            $_POST['brand_banner'] = $upload->file_name;
                                    }
                            }
//                                //上传图片3
//                            $upload->set('default_dir',ATTACH_BRAND);
...
//                            $upload->set('file_name', '');
//                            $upload->set('ifremove', false);
//                            if (!empty($_FILES['brand_poster']['name'])){
//                                    $result = $upload->upfile('brand_poster');
//                                    if (!$result){
//                                            showMessage($upload->error,'','','error');
//                                    }else{
//                                            $_POST['brand_poster'] = $upload->file_name;
//                                    }
//                            }
//                                    //上传图片4
//                            $upload->set('default_dir',ATTACH_BRAND);
...
//                            $upload->set('file_name','');
//                            $upload->set('ifremove',false);
//                            if (!empty($_FILES['brand_pic1']['name'])){
//                                    $result = $upload->upfile('brand_pic1');
//                                    if (!$result){
//                                            showMessage($upload->error,'','','error');
//                                    }else{
//                                            $_POST['brand_pic1'] = $upload->file_name;
//                                    }
//                            }
//                                    $insert_array = array();
                                    $insert_array['brand_name'] = trim($_POST['brand_name']);
                                    $insert_array['brand_initial'] = strtoupper($_POST['brand_initial']);
                                    $insert_array['buyer_name'] = trim($_POST['buyer_name']);
                                    $insert_array['buyer_yuju'] = trim($_POST['buyer_yuju']);
                                    $insert_array['brand_country'] = trim($_POST['brand_country']);
                                    ...
                                    $insert_array['brand_banner'] = trim($_POST['brand_banner']);
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['brand_name'] = trim($_POST['brand_name']);
                                $update_array['brand_country'] = trim($_POST['brand_country']);
                                $update_array['buyer_name'] = trim($_POST['buyer_name']);
                                $update_array['buyer_yuju'] = trim($_POST['buyer_yuju']);
				...
				if (!empty($_POST['brand_pic'])){
					$update_array['brand_pic'] = $_POST['brand_pic'];
                                ...
                                if (!empty($_POST['brand_banner'])){
					$update_array['brand_banner'] = $_POST['brand_banner'];
				}
//                                if (!empty($_POST['brand_poster'])){
//					$update_array['brand_poster'] = $_POST['brand_poster'];
//				}
//                                if (!empty($_POST['brand_pic1'])){
//					$update_array['brand_pic1'] = $_POST['brand_pic1'];
//				}
				...
				if ($result){
					if (!empty($_POST['brand_pic']) && !empty($brand_info['brand_pic'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.$brand_info['brand_pic']);
                                        ...
                                        if (!empty($_POST['brand_banner']) && !empty($brand_info['brand_banner'])){
						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.$brand_info['brand_banner']);
					}
//                                        if (!empty($_POST['brand_poster']) && !empty($brand_info['brand_poster'])){
//						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.$brand_info['brand_poster']);
//					}
//                                        if (!empty($_POST['brand_pic1']) && !empty($brand_info['brand_pic1'])){
//						@unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.$brand_info['brand_pic1']);
//					}
					$url = array(
						array(
