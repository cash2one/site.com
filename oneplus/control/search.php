...
class searchControl extends SystemControl{
	...
	public function searchOp(){
		...
		Tpl::showpage('setting.search');
        	...
        	public function search_brandOp(){
		if (chksubmit()){
			$lang	= Language::getLangContent();
			$model_setting = Model('setting');
			...
			$result = $model_setting->updateSetting(array(
				'hot_search_brand'=>str_replace($comma,',',$_POST['hot_search_brand'])));
			if ($result){
				showMessage($lang['nc_common_save_succ']);
			}else {
				showMessage($lang['nc_common_save_fail']);
		...
		Tpl::showpage('setting.search_brand');
	}
		...
		if (chksubmit()){
			...
			$result = $model_setting->updateSetting(array(
				'hot_search_store'=>str_replace($comma,',',$_POST['hot_search_store'])));
			if ($result){
				showMessage($lang['nc_common_save_succ']);
			}else {
				showMessage($lang['nc_common_save_fail']);
		...
		Tpl::showpage('setting.search_store');
	}
