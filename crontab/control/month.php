...
class monthControl extends BaseCronControl {
    ...
    private function _real_order() {
        $settlement_date = C('settlement_date'); //结算日
        $model_order = Model('order');
        $model_bill = Model('bill');
        //最后的生成时间
        $order_statis_max_info = $model_bill->getOrderStatisInfo(array(),'os_end_date','os_end_date desc');
        ...
        while (($time = strtotime('-'.$i.' month',$current_time)) >= $start_unixtime) {
            foreach ($settlement_date as $k=>$v){
                $settlementday = date('Y-m',$time)."-".$v;
                $settlementtime = strtotime($settlementday)-1;
                if(TIMESTAMP < $settlementtime){
                    break;
                }
                $set_time[]=$settlementtime;
            }
            $i++;
        }
        sort($set_time);
        for ($i=0;$i<sizeof($set_time)-1;$i++){

            $first_day_unixtime = $set_time[$i]+1;
            $last_day_unixtime = $set_time[$i+1];
            $key = count($data);
            $os_month = date('Ymd',$last_day_unixtime+1);
    ...
    private function _create_real_order_bill($data){
        ...
        $store_count = $model_store->getStoreCount(array()); //92数量
