...
<div class="ncsc-form-default">
  <form id="add_form" action="index.php?act=store_groupbuy&op=groupbuy_save" method="post" enctype="multipart/form-data">
    ...
    <dl>
      <dt><i class="required">*</i>抢购类型：</dt>
      <dd>
        <select id="groupbuy_type" name="groupbuy_type" class="w80">
            <option value="">请选择</option>
            <option value="0">限时抢</option>
            <option value="1">限量抢</option>
        </select>
        <span></span>
        <p class="hint">默认限时抢购</p>
      </dd>
    </dl>
    <dl>
...
<?php } ?>
            ...
            groupbuy_name: {
                required : '<i class="icon-exclamation-sign"></i>请选择抢购类型'
            },
        }
    });
