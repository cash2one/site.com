...
<a href="<?php echo U('Crowdfunding/index',array('token'=>$token));?>"  class="right btnGrayS vm" >返回</a>
<div class="clr"></div>
</div>
...
<div class="searchbar right">
	<form method="post" action="">
		<input type="text" id="msgSearchInput" class="txt left" placeholder="输入订单号" name="search" value="">
...
<td>
	<?php if($order["reward_id"] == -1): ?>无私奉献
	<?php else: ?>
		支持<?php endif; ?>
</td>
<td>
	<?php if($order["paid"] == 1): ?>支付完成
	<?php else: ?>
		未支付<?php endif; ?>
</td> 
<td>
...
<td class="norightborder">
	<a href="<?php echo U('Crowdfunding/order_detail',array('id'=>$order['id'],'token'=>$token));?>">订单详情</a>&nbsp;
	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Crowdfunding/order_del',array('token'=>$token,'pid'=>$order['pid'],'id'=>$order['id']));?>');">删除</a></td>
