			...
			<tbody>
				<?php if(is_array($service_list)): $i = 0; $__LIST__ = $service_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td class="norightborder" style="border-right:1px solid #eee">
							<a href="<?php echo U('Service/update',array('token'=>$token,'type'=>'setup','id'=>$vo['imicms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["nickname"]); ?>】吗?', '<?php echo U('Service/del',array('token'=>$token,'id'=>$vo['imicms_id'],'type'=>'setup'));?>')">删除</a>
						</td>
