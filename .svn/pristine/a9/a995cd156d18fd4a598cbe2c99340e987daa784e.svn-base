...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $order_pay_info = $model_order->getOrderPayInfo($condition);//return callback(true,'',$order_pay_info);
    ...
    //计算本次需要在线支付的订单总金额
        ...
        if (!empty($order_list)) {
            foreach ($order_list as $order_info) {
		        ...
		        $payed_amount = floatval($order_info['rcb_amount']) + floatval($order_info['pd_amount'] +  floatval($order_info['onempf_amount']));
     ...
     * @return array
     */
    public function getCardOrderInfo($pay_sn, $member_id = null) {
    
        //验证订单信息
        $model_order = Model('card_combination');
        $condition = array();
        $condition['pay_sn'] = $pay_sn;
        if (!empty($member_id)) {
            $condition['create_member_id'] = $member_id;
        }
        $order_info = $model_order->getOne($condition);
        if(empty($order_info)){
            return callback(false,'该订单不存在');
        }

        $order_info['subject'] = '礼品卡订单_'.$pay_sn;
        $order_info['order_type'] = 'card_order';
        $order_info['pay_sn'] = $pay_sn;


         //计算所需要支付金额(支付卡+余额支付了)//zmr>v60
        $pay_amount = floatval($order_info['card_price']);
    ...
    public function getOrderExtendList(& $order_list,$role = '') {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            //当以下情况时不需要清除数据pd_amount,rcb_amount,onempf_amount：
            //如果第2次支付尾款，并且已经锁定了站内款
            //当以下情形时清除站内余额数据pd_amount,rcb_amount,onempf_amount：
            ...
            if (!$order_list[0]['if_buyer_pay_lock']) {
                $order_list[0]['pd_amount'] = $order_list[0]['rcb_amount'] = $order_list[0]['onempf_amount'] = 0;
            }
        }
     ...
     * 支付成功后修改礼品卡订单状态
     */
    public function updateCardOrder($out_trade_no, $payment_code, $order_info, $trade_no) {
        $post['payment_code'] = $payment_code;
        $post['trade_no'] = $trade_no;
        return Logic('card_combination')->changeOrderStatePay($order_info, 'system', $post);
    }
