...
<div class="content" >  
<div class="cLineB">
  <h4><?php if($sceneid == 1): ?>微现场<?php else: ?>微信墙<?php endif; ?>信息审核</h4>
...
<form action="" method="post">
  <div class="checked">
    审核模式： <img src="/tpl/static/wall/images/<?php if($ck_msg == '1'): ?>open.png<?php else: ?>close.png<?php endif; ?>" alt="">
  </div>
...
<div id="div_ptype" class="member-list">
  <table class="ListProduct" width="100%" cellspacing="0" cellpadding="0" border="0">
    ...
    <tbody id="msg">
      <?php if($list): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($info["id"]); ?>">
          <td align="left">
            <div class="con">
              <?php if($info["picture"] == ''): echo ($info["content"]); ?>
              <?php else: ?>
                <a href="?g=User&m=Wechat_group&a=showExternalPic&url=<?php echo ($info["picture"]); ?>&wecha_id=<?php echo ($info["id"]); ?>" target="_blank" class="pic">
                  <img src="?g=User&m=Wechat_group&a=showExternalPic&url=<?php echo ($info["picture"]); ?>&wecha_id=<?php echo ($info["id"]); ?>" alt="">
                </a><?php endif; ?>
            </div>
          </td>
          <td align="center"><?php echo ($info["username"]); ?></td>
          <td align="center"><?php echo (date("Y-m-d H:i:s",$info["time"])); ?></td>
          <td align="center">
              <?php if($info["is_check"] == '1'): ?><i class="dui"  onclick="is_check(this,<?php echo ($info["id"]); ?>)"></i>
              <?php else: ?>
                <i class="cuo"  onclick="is_check(this,<?php echo ($info["id"]); ?>)"></i><?php endif; ?>  
          </td>
          <td align="center"><a href="javascript:void(0);" onclick="del_msg(<?php echo ($info["id"]); ?>)">删除</a></td>
...
<input type="hidden" name="loadtime" id="loadtime" value="<?php echo ($now); ?>">
<script>
...
function getMsg(){
  ...
  $.getJSON('<?php echo U('Wall/laodMsg',array('token'=>$_SESSION['token']));?>', {'loadtime':loadtime,'id':id,'sceneid':sceneid,'uid':uid}, function(data){
       if(data.err == 0){  
            var html = '';
...
function del_msg(mid){
    if(confirm('您确定要删除这条信息么?')){
      $.getJSON('<?php echo U('Wall/del_msg',array('token'=>$_SESSION['token']));?>', {'wallid':<?php echo ($id); ?>,'sceneid':sceneid,'mid':mid}, function(data){
        if(data){
...
function is_check(obj,mid){
  ...
  var checked = '';
    ...
    $.post('<?php echo U('Wall/is_check',array('token'=>$_SESSION['token']));?>', {'wallid':<?php echo ($id); ?>,'sceneid':sceneid,'mid':mid,'checked':checked}, function(data){
      var data = eval("("+data+")");
      if(data.err == 1){
        alert(data.info);
      }else if(data.err == 0){
