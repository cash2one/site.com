...
<div class="content">
<div class="cLineB">
<h4 class="left">微信现金红包</h4>
<div class="clr"></div>
</div>
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('Cashbonus/set',array('token'=>$token));?>" title="添加红包" class="btnGrayS vm bigbtn">添加现金红包</a> </div>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<td>手气红包<br><?php echo ($list["model"]); ?></td>
	<td>
		发放个数：<?php echo ($list["logcount"]); ?> 个<br/>
		　总金额：<?php echo ($list["item_sum"]); ?> 元<br />
		面额上限：<?php echo ($list["item_max"]); ?> 元
	</td>
	<td>
		可领：<?php echo ($list["people"]); ?>人<br />
		<?php if($list["people"] > 0): ?>已领：<?php echo ($list["pcount"]); ?>人<br />
		已领：<?php echo ($list["logprice"]); ?> 元<?php endif; ?>
	</td>
	<td>
	...
	<td class="norightborder" align="center">
		<a href="<?php echo U('Cashbonus/prize_log',array('token'=>$token,'id'=>$list['id']));?>">领取记录</a>
		|
		<a href="<?php echo U('Cashbonus/set',array('token'=>$token,'id'=>$list['id']));?>">修改</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Cashbonus/del',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
