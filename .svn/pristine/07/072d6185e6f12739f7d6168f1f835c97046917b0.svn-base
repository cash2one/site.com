...
class groupbuyControl extends SystemControl {
     ...
     * 规则
     * */
    public function groupbuy_rulesOp() {

        $model_setting = Model('setting');
        $setting = $model_setting->GetListSetting();
        Tpl::output('setting', $setting);

        $this->show_menu('groupbuy_rules');
        Tpl::showpage('groupbuy.rules');
    }

    public function groupbuy_bannerOp() {
        $model_setting = Model('setting');
         $this->show_menu('groupbuy_rules');
        Tpl::showpage('groupbuy.banner');
    }

    public function groupbuy_rules_saveOp() {

        $model_setting = Model('setting');
        $update_array = array();
        $update_array['groupbuy_rules'] = $_POST['groupbuy_rules'];
    ...
    public function banner_uploadOp() {
        ...
        if (!empty($_FILES['up_pic']['name'])) {
            ...
            $param['groupbuy_banner'] = $upload->getSysSetPath() . $upload->file_name;
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            ...
            'groupbuy_rules' => array('menu_type' => 'link', 'menu_name' => '抢购规则', 'menu_url' => urlAdmin('groupbuy', 'groupbuy_rules')),
            'groupbuy_banner' => array('menu_type' => 'link', 'menu_name' => 'banner上传', 'menu_url' => urlAdmin('groupbuy', 'groupbuy_banner')),
