...
function drop_topcart_item(cart_id,goods_id){
    ...
    $.getJSON('/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.head-user-menu .my-cart');
            //删除成功
	...
	$.fn.membershipCard = function(options){
		...
		function follow(o){
			...
			$.getJSON(SHOP_SITE_URL+'/index.php?act=member_snsfriend&op=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
					$('[nctype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" >已+1</a> <a href="javascript:void(0);" nctype="nofollow">取消关注</a>').find('a[nctype="nofollow"]').click(function(){
						onfollow($(this));
					});
				}
			});
...
/* 加入购物车 */
function addcart_id(goods_id,quantity,callbackfunc,id) {
    var url = 'index.php?act=cart&op=add';
    quantity = parseInt(quantity);
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
...
//抢购循环倒计时
function groupbyCountdown(){
    var date = new Date();
    var time = date.getTime();  //当前时间距1970年1月1日之间的毫秒数
     
    $(".settime").each(function(i){
     
        var startDate =$(this).attr("startTime"); //开始时间字符串
        ...
        var startTime = startDate1.getTime(); //结束时间毫秒数
    ...
    setTimeout("groupbyCountdown()",1000);
}
