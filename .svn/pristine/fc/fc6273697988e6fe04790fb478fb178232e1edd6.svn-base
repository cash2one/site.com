...
<div class="content">
	...
	<?php if($wxuser["winxintype"] == 3): ?><div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-top:5px;">
...
3、审核中的门店暂不支持修改和删除。
	...
	<div class="tab">
		<ul>
			...
			<?php if($wxuser["wx_coupons"] == 1): ?><!--li style="float:right;"><a href="<?php echo U('Company/updateallpoi',array('token'=>$token));?>">更新同步门店</a></li--><?php endif; ?>
			<?php if($set["id"] != '' && $isBranch != 1): ?><li style="float:right;">
			<?php if($set["available_state"] == 2 or $set["available_state"] == 0): ?><a href="javascript:drop_confirm('门店在审核中,你确定要删除吗?', '<?php echo U('Company/delete',array('token'=>$token,'id'=>$set['id']));?>');">删除公司信息</a>
			<?php else: ?>
			<a href="javascript:drop_confirm('你确定要删除吗?', '<?php echo U('Company/delete',array('token'=>$token,'id'=>$set['id']));?>');">删除公司信息</a><?php endif; ?>
			</li><?php endif; ?>
		</ul>
	</div>
	<!--tab end-->
			...
			<div class="msgWrap bgfc"> 
				<table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
					...
					<?php if($set["id"] == ''): ?><tr> 
						...
						<td><input type="text" id="name" name="name" value="<?php echo ($set["name"]); ?>" class="px require" style="width:400px;" />&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:red;">名称添加后不可更改</span></td> 
					</tr>
					<?php else: ?>
						<tr> 
						<th><span class="red"></span>名称：</th> 
					...
					<?php if($set["id"] == ''): ?><tr>
						...
						<tr>
						<th>行业分类：</th>
						<td><?php echo ($set["cat_name"]); ?></td></tr><?php endif; ?>
					<?php if($category_list != ''): if($set["id"] == ''): ?><tr>
							<th><span class="red">*</span>门店类型：</th>
							<td>
								<select id="categories" autocomplete="off" name="categories" style="width:200px;">
								<?php if(is_array($category_list)): $i = 0; $__LIST__ = $category_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><option value="<?php echo ($category); ?>" <?php if($set["categories"] == $category): ?>selected="selected"<?php endif; ?> ><?php echo ($category); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						...
						<tr>
						<th><span class="red">*</span>门店类型：</th>
						<td><?php echo ($set["categories"]); ?></td></tr><?php endif; endif; ?>
						<tr> 
						...
						<tr> 
							...
							<td>
								<input id="radio1" class="radio" type="radio" name="display" value="1"  <?php if($set["display"] == '1' or $set["display"] == ''): ?>checked<?php endif; ?>> <label for="radio1">显示</label>
								<input class="radio" id="radio2" type="radio" name="display" value="0"   <?php if($set["display"] == '0'): ?>checked<?php endif; ?>> <label for="radio2">隐藏</label>
							</td>
						</tr><?php endif; ?>
						<?php if($set["id"] == ''): ?><tr> 
						...
						<tr>
						<th>门店地址：</th>
						<td><?php echo ($set["province"]); ?>---<?php echo ($set["city"]); ?>---<?php echo ($set["district"]); ?></td></tr><?php endif; ?>
						<?php if($set["id"] == ''): ?><tr> 
							<th><span class="red">*</span>详细地址：</th> 
							<td><input type="text" id="address" name="address" value="<?php echo ($set["address"]); ?>" class="px require" style="width:400px;" />&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:red;">详细地址添加后不可更改</span></td> 
						</tr> 
						<?php else: ?>
						<tr><th>详细地址：</th><td><?php echo ($set["address"]); ?></td></tr><?php endif; ?>
						<tr> 
						...
						<?php if($set["id"] == ''): ?><tr> 
							...
							<td>经度 <input type="text" id="longitude"  name="longitude" size="14" class="px" value="<?php echo ($set["longitude"]); ?>" /> 纬度 <input type="text"  name="latitude" size="14" id="latitude" class="px" value="<?php echo ($set["latitude"]); ?>" /> <a href="###" onclick="setlatlng($('#longitude').val(),$('#latitude').val())">在地图中查看/设置</a>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:red;">经纬度添加后不可更改</span></td> 
						</tr>
						<?php else: ?>
						<tr><th>经纬度：</th><td>经度：<?php echo ($set["longitude"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;纬度：<?php echo ($set["latitude"]); ?></td></tr><?php endif; ?>
						<?php if($isBranch == 1): ?><tr> 
						...
						<tr>
							...
							<?php $__FOR_START_2002966576__=1;$__FOR_END_2002966576__=25;for($i=$__FOR_START_2002966576__;$i < $__FOR_END_2002966576__;$i+=1){ ?><option value="<?php echo ($i); ?>" <?php if($set["opentime"] != '' && $set["opentime"] == $i): ?>selected = selected<?php endif; ?>><?php echo ($i); ?>:00</option><?php } ?>
							</select>---<select name="closetime" style="width:100px;">
							<?php $__FOR_START_774472774__=1;$__FOR_END_774472774__=25;for($i=$__FOR_START_774472774__;$i < $__FOR_END_774472774__;$i+=1){ ?><option value="<?php echo ($i); ?>" <?php if($set["closetime"] != '' && $set["closetime"] == $i): ?>selected = selected<?php endif; ?>><?php echo ($i); ?>:00</option><?php } ?>
							</select></td>
							</tr>
						<tr> 
							<th><span class="red">*</span>人均消费：</th> 
							<td><input type="text" id="avg_price" name="avg_price" value="<?php echo ($set["avg_price"]); ?>" class="px require" style="width:100px;" /></td> 
						</tr>
						<tr> 
							<th>特色服务：</th> 
							<td><input type="text" id="special" name="special" value="<?php echo ($set["special"]); ?>" class="px require" style="width:400px;" /></td> 
						</tr>
						<tr> 
							<th>商户简介:</th> 
							<td><input type="text" id="introduction" name="introduction" value="<?php echo ($set["introduction"]); ?>" class="px require" style="width:400px;" /></td> 
						</tr>
						<tr> 
							<th>推荐品:</th> 
							<td><input type="text" id="recommend" name="recommend" value="<?php echo ($set["recommend"]); ?>" class="px require" style="width:400px;" /></td> 
						</tr>
						<tr> 
						...
						<?php if($set["id"] != '' && $wxuser["winxintype"] == 3): ?><tr>
							<th valign="top"><label for="info">状态：</label></th>
							<td><?php if($set['available_state'] == 2 || $set['available_state'] == 0): ?><span style="color:red;">审核中</span><?php elseif($set['available_state'] == 3): ?><span style="color:green;">审核通过</span><?php elseif($set['available_state'] == 4): ?><span style="color:red;">审核驳回</span><?php endif; ?></td>
						</tr><?php endif; ?>
						<tr>         
							<th>&nbsp;</th>
							<td>
							<?php if($isBranch == 1): ?><input type="hidden" name="isbranch" value="1" /><?php endif; ?>
							<?php if($set["id"] != ''): ?><input type="hidden" name="sid" value="<?php echo ($set["id"]); ?>" /><?php endif; ?>
