  ...
  <tbody>
    <tr><th><label><?php echo $lang['belongs_level'];?></label></th>
      <td><select name="grade_id">
        ...
        <th><label><?php echo $lang['belongs_class'];?></label></th>
        <td><select name="sc_id">
                <option value="0"><?php echo $lang['nc_please_choose'];?>...</option>
                <?php if(is_array($output['class_list'])){ ?>
                    <?php foreach($output['class_list'] as $k => $v){ ?>
                        <option <?php if($output['sc_id'] == $v['sc_id']){ ?>selected="selected"<?php } ?> value="<?php echo $v['sc_id']; ?>"><?php echo $v['sc_name']; ?></option>
                    <?php } ?>
                <?php } ?>
            </select></td>
        <th><label for="owner_and_name"><?php echo $lang['store_user'];?></label></th>
      <td><input type="text" value="<?php echo $output['owner_and_name'];?>" name="owner_and_name" id="owner_and_name" class="txt"></td><td></td><th><label><?php echo $lang['state'];?></label></th>
        <td>
            <select name="store_state">
                <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
                <?php if(!empty($output['store_state']) && is_array($output['store_state'])){ ?>
                <?php foreach($output['store_state'] as $k => $v){ ?>
                <option value="<?php echo $k;?>" <?php if($_GET['store_state'] == $k){?>selected<?php }?>><?php echo $v;?></option>
                <?php } ?>
                <?php } ?>
            </select>
        </td>
      ...
      <td><input type="text" value="<?php echo $output['store_name'];?>" name="store_name" id="store_name" class="txt"></td>
        <th><label><?php echo $lang['is_recommend'];?></label></th>
        <td>
            <select name="is_recommend">
                <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
                <option value="1" <?php if($_GET['is_recommend'] === "1"){?>selected<?php }?>>是</option>
                <option value="0" <?php if($_GET['is_recommend'] === "0"){?>selected<?php }?>>否</option>
            </select>
        </td>
        <th><label><?php echo $lang['is_brand'];?></label></th>
        <td>
            <select name="is_brand">
                <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
                <option value="1" <?php if($_GET['is_brand'] === "1"){?>selected<?php }?>>是</option>
                <option value="0" <?php if($_GET['is_brand'] === "0"){?>selected<?php }?>>否</option>
            </select>
        </td>
        <th><label><?php echo $lang['is_artisan'];?></label></th>
        <td>
            <select name="is_artisan">
                <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
                <option value="1" <?php if($_GET['is_artisan'] === "1"){?>selected<?php }?>>是</option>
                <option value="0" <?php if($_GET['is_artisan'] === "0"){?>selected<?php }?>>否</option>
            </select>
        </td>
    ...
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
          ...
          <th class="align-center"><?php echo $lang['is_recommend'];?></th>
          <th class="align-center"><?php echo $lang['is_brand'];?></th>
          <th class="align-center"><?php echo $lang['is_artisan'];?></th>
      ...
      <tbody>
        ...
        <tr class="hover edit <?php echo getStoreStateClassName($v);?>">
          ...
          <td class="yes-onoff align-center"><a href="JavaScript:void(0);" class=" <?php echo $v['is_recommend']? 'enabled':'disabled'?>" ajax_branch='is_recommend'  nc_type="inline_edit" fieldname="is_recommend" fieldid="<?php echo $v['store_id']?>" fieldvalue="<?php echo $v['is_recommend']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
          <td class="yes-onoff align-center"><a href="JavaScript:void(0);" class=" <?php echo $v['is_brand']? 'enabled':'disabled'?>" ajax_branch='is_brand'  nc_type="inline_edit" fieldname="is_brand" fieldid="<?php echo $v['store_id']?>" fieldvalue="<?php echo $v['is_brand']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
          <td class="yes-onoff align-center"><a href="JavaScript:void(0);" class=" <?php echo $v['is_artisan']? 'enabled':'disabled'?>" ajax_branch='is_artisan'  nc_type="inline_edit" fieldname="is_artisan" fieldid="<?php echo $v['store_id']?>" fieldvalue="<?php echo $v['is_artisan']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
