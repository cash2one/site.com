...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id,$is_happysend) {
        ...
        $result = $this->getBuyStep1Data($member_id,$result['data'],$is_happysend);
        return $result;
    }
    ...
    /** * 第一步：返回商品、促销、地址、发票等信息，然后交前台抛出
     * @param unknown $member_id
     * @param unknown $data 商品信息
     * @return
     * edit by niro
     */
    public function getBuyStep1Data($member_id, $data,$is_happysend) {
        $store_id = $data['goods_list'][0]['store_id'];
    ...
    private function _createOrderStep1() {
        ...
        if ($input_address_id <= 0 && $post['is_happysend']==0) {
            throw new Exception('请选择收货地址');
        }else if($input_address_id >0){
        ...
        if (!in_array($input_if_offpay,array('allow_offpay','deny_offpay')) && $post['is_happysend']===0) {
        ...
        if (!is_array($input_if_offpay_batch)  && $post['is_happysend']===0) {
        ...
        if($post['share_id']){
        $this->_order_data['share_id'] = $post['share_id'];//分享人的id  addbyniro
        }
        $this->_order_data['is_happysend'] = $post['is_happysend'];//是否欢乐送  addbyniro 
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            if($share_id){
            $order['share_id'] = $share_id;//分享人的id add by niro
            }
            ...
            if($is_happysend==0||$is_happysend==''){
	...
	/**
	 * 赠送礼品卡
	 * add by lizh 15:09 2016/8/3
	 */
	public function create_card_order($order_data,$member_id,$member_name,$member_email) {
		
		$this->_member_info['member_id'] = $member_id;
        $this->_member_info['member_name'] = $member_name;
        $this->_member_info['member_email'] = $member_email;
        $this->_post_data = $order_data;
		...
		try {
			...
			//创建支付单号
			$pay_sn = $this->_logic_buy_1->cardMakePaySn($member_id);
			$this->_post_data['pay_sn'] = $pay_sn;
			$this->_order_data['pay_sn'] = $pay_sn;
			
			//创建礼品卡订单
			$combination_data = $this -> _createCardOrderStep1();
			$this->_order_data['combination_data'] = $combination_data;
            $model->commit();
	...
	/**
	 * 创建礼品卡订单
	 * add by lizh 16:49 2016/8/3
	 */
	private function _createCardOrderStep1() {
		
		$post = $this->_post_data;
		
        $insert_array['order_from'] = $post['order_from'];                      	//订单来源：1为PC 2为手机
        $insert_array['quantity'] = $post['quantity'];                              //礼品卡数量
		
        $insert_array['member_id'] = $post['member_id'];                            //礼品卡所属用户ID
        $insert_array['member_mobile'] = $post['member_mobile'];                    //礼品卡所属用户手机号
        $insert_array['card_price'] = $post['card_price'];                          //礼品卡金额
        $insert_array['card_id'] = $post['card_id'];                                //礼品卡ID
        $insert_array['use_state'] = $post['use_state'];                            //使用状态
        $insert_array['create_time'] = $post['create_time'];                        //创建时间
        $insert_array['content'] = $post['content'];                                //内容
        $insert_array['bill_state'] = $post['bill_state'];                          //支付状态
        $insert_array['create_member_id'] = $post['create_member_id'];              //创建用户
		$insert_array['pay_sn'] = $post['pay_sn'];                                  //支付单号
		
		$card_combination = Model('card_combination');
		$combination_id = $card_combination -> save($insert_array);
		
		$combination_data = $card_combination ->getOne(array(combination_id => $combination_id));
