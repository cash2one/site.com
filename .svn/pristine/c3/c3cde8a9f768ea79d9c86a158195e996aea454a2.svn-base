    ...
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
            ...
            <?php } ?>
            <?php if($_GET['op'] == 'cms_article_list') { ?>
            <th><label for="article_state">类别：</label></th>
            <td>
                <select name="article_class_id">
                    <option value="0"><?php echo $lang['nc_please_choose'];?></option>
                    <?php if(!empty($output['cms_article_class_data']) && is_array($output['cms_article_class_data'])) {?>
                    <?php foreach($output['cms_article_class_data'] as $key=>$value) {?>
                        <option value="<?php echo $value['class_id'];?>" <?php if($value['class_id']==$_GET['article_class_id']) { echo 'selected'; } ?> ><?php echo $value['class_name'];?></option>
                    <?php } ?>
                    <?php } ?>
                </select>
            </td>
        ...
        <tr class="hover edit">
          ...
          <td class="w60 align-center yes-onoff"><a href="JavaScript:void(0);" class=" <?php echo $val['article_attitude_flag']? 'enabled':'disabled'?>" ajax_branch='article_attitude'  nc_type="inline_edit" fieldname="article_attitude_flag" fieldid="<?php echo $val['article_id']?>" fieldvalue="<?php echo $val['article_attitude_flag']?'1':'0'?>" title="<?php echo $lang['editable'];?>"><img src="<?php echo ADMIN_TEMPLATES_URL;?>/images/transparent.gif"></a></td>
            <td class="w60 align-center">
                <?php echo $output['article_state_list'][$val['article_state']]['text'];?>
                <!-- <select name="article_state" id="article_state">
                    <option value="">请选择...</option>
                    <?php if(is_array($output['article_state_list']) && !empty($output['article_state_list'])){?>
                    <?php foreach ($output['article_state_list'] as $k => $v) { ?>
                        <option value="<?php echo $k;?>" <?php if($val['article_state'] == $k){ ?>selected='selected'<?php } ?>><?php echo $v['text'];?></option>
                       <?php echo $k,'/',$val['article_state']; ?>  
                    <?php } ?>
                    <?php } ?>
                </select> -->
            </td>
            <td class="w108 align-center">
                ...
                <?php } ?>
                <a href="index.php?act=cms_article&op=cms_article_edit&article_id=<?php echo $val['article_id']; ?>"><?php echo $lang['nc_edit'];?></a>
                <a href="javascript:submit_delete(<?php echo $val['article_id'];?>)"><?php echo $lang['nc_del'];?></a>
            </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
