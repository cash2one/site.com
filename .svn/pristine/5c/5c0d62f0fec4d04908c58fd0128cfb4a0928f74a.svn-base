...
class goodsModel extends Model{
    ...
    public function getIsrecommendGoods($condition,$limit){
      ...
      // $sql = "SELECT goods_commonid FROM wantease_goods_common WHERE store_id=123 ORDER BY is_recommend DESC LIMIT 5";

        $goods_commonid_arr =  $this->table('goods_common')->field("goods_commonid")->order('is_recommend DESC')->where($condition)->limit($limit)->select();
