...
class Control{
     ...
     *
     */
    protected function checkUserMamoryLogin(){
        //客户端IP
        $client_ip = getIp();
        ...
        $member_info = cookie($client_ip);
        if(!empty($member_info)) {
            
            if($_SESSION['is_login'] != '1') {
                
                $model_member = Model('member');
                $member_info = unserialize($member_info);
		...
		$gc_parent_id = $all_goods_class_array[0]['gc_id'];
		$down_goods_class_array = Model('goods_class') -> where(array(gc_parent_id => $gc_parent_id )) -> select();
		Tpl::output('down_goods_class_array', $down_goods_class_array);
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
				...
				array('name' => '瞬间认领', 'act'=>'store_inspiration', 'op'=>'index'),
                array('name' => '我的认领', 'act'=>'my_inspiration', 'op'=>'index'),
            )),
...
class BaseBiaojuControl extends Control {
