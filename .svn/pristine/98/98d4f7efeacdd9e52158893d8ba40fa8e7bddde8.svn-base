...
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('FrontPage/addaction',array('token'=>$token));?>" title="添加活动" class="btnGrayS vm bigbtn">添加活动</a> </div>  <div class="pageNavigator left"><a href="<?php echo U('FrontPage/listconfigure',array('token'=>$token));?>" title="配置信息" class="btnGrayS vm bigbtn">配置信息</a> </div>
<div class="searchbar right">
<form method="post" action="<?php echo U('FrontPage/index',array('token'=>$token));?>">
<input type="text" id="msgSearchInput" class="txt left" placeholder="输入活动名称或关键词搜索" name="search_word" value="<?php echo ($search_word); ?>" style="padding:6px;width:155px;">
<input type="submit" value="搜索" id="msgSearchBtn" href="" class="btnGrayS" title="搜索">
</form>
</div>
<div class="clr"></div>
</div>
<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;font-size:12px;"><p>温馨提示：需要在配置信息里面填写好相应信息才可正常使用。</p></div>
<div class="msgWrap">
...
<tbody>
<?php if($list != ''): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	<td><?php echo ($list["action_name"]); ?></td>
	<td><?php echo ($list["keyword"]); ?></td>
	<td><?php echo ($list["sharecount"]); ?></td>
	<td>
		<?php if($list["status"] == '1'): ?><span style="color:green;">进行中</span>
		<?php else: ?>
			<span style="color:red;">已关闭</span><?php endif; ?>
	</td>
	<td class="norightborder" align="center">
		<a href="<?php echo U('FrontPage/fansnews',array('token'=>$list['token'],'id'=>$list['id'],'news_type'=>2));?>">粉丝新闻列表</a>
		|		
		<a href="<?php echo U('FrontPage/systemnews',array('token'=>$token,'id'=>$list['id'],'news_type'=>1));?>">随机事件列表</a>
		|
		<a href="<?php echo U('FrontPage/addaction',array('token'=>$list['token'],'id'=>$list['id']));?>">编辑</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('FrontPage/delaction',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
</tr><?php endforeach; endif; else: echo "" ;endif; ?>
<?php else: ?>
<tr><td colspan="5">暂无记录</td></tr><?php endif; ?>
</tbody>
</table>
</div>
<div class="cLine">
