...
//        $condition['order_state'] = $_GET['order_state'];
        $order_list_array = $model_order->getNormalOrderList($condition, $this->page, '*', 'order_state desc','', array('order_goods'));
...
//print_r($payment_list);
    ...
    public function order_cancelOp() {
     ...
     public function order_ensureOp() {
         $model_order = Model('order');
        $logic_payment = Logic('payment');
      $pay_sn = intval($_GET['pay_sn']);
     
       $update_order = array();
            $update_order['order_state'] = ORDER_STATE_PAY;
            $update_order['payment_time'] = ($_GET['payment_time'] ? strtotime($_GET['payment_time']) : TIMESTAMP);
            $update_order['payment_code'] = $_GET['payment_code'];
             $update = $model_order->insert($update_order,array('pay_sn'=>$pay_sn));
