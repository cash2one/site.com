...
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('ShakeLottery/AddShakeLottery');?>" title="添加摇一摇中奖活动" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 添加活动</a> </div>
<div class="searchbar right">
<form method="post" action="<?php echo U('ShakeLottery/index',array('token'=>$token));?>">
...
<?php if($list != ''): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<td><?php echo (date("Y-m-d H:i",$list["starttime"])); ?><br /><?php echo (date("Y-m-d H:i",$list["endtime"])); ?></td>
	<td><?php echo ($list["share_count"]); ?></td>
	<td>
		<?php if($list["status"] == '1'): ?><span style="color:green;">活动已开启</span>
		<?php else: ?>
			<span style="color:red;">活动已关闭</span><?php endif; ?>
	</td>
	<td class="norightborder" align="center">
		<a href="<?php echo U('ShakeLottery/PrizeList',array('token'=>$list['token'],'aid'=>$list['id']));?>">摇奖奖品</a>
		|
		<a href="<?php echo U('ShakeLottery/ShakeLotteryRecord',array('token'=>$list['token'],'aid'=>$list['id']));?>">摇奖记录</a>
		|
		<a href="<?php echo U('ShakeLottery/AddShakeLottery',array('id'=>$list['id']));?>">编辑</a>
		|
		<a href="javascript:drop_confirm('删除活动该活动的奖品和领奖记录会一并删除，您确定要删除吗?', '<?php echo U('ShakeLottery/DelShakeLottery',array('id'=>$list['id']));?>');">删除</a>
    </td>
</tr><?php endforeach; endif; else: echo "" ;endif; ?>
<?php else: ?>
<tr><td colspan="6">暂无记录</td></tr><?php endif; ?>
</tbody>
</table>
</form>
</div>
