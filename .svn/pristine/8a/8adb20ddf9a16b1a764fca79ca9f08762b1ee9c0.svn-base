	  ...
	  <td>ID</td>
	    ...
	    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			<td ><?php echo ($vo["id"]); ?></td>
				...
				<td align='center'>
					<a href="<?php echo U('Agent/add/',array('id'=>$vo['id']));?>">修改</a> <a href="###" onclick="if(confirm('确定要删除吗')){location.href='<?php echo U('Agent/del/',array('id'=>$vo['id']));?>';}" >删除</a> <a href="?g=System&m=Users&a=index&agentid=<?php echo ($vo["id"]); ?>">账号列表</a> <a href="?g=System&m=Token&a=index&agentid=<?php echo ($vo["id"]); ?>">公众号列表</a> <?php if($vo['level'] == 0): ?><a href="?g=System&m=Agent&a=index&agentid=<?php echo ($vo["id"]); ?>">代理商列表</a><?php endif; ?>
				</td>
