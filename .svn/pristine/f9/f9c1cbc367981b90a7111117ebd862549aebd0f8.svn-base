              ...
              <TBODY>
				...
				<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "没有找到数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                 ...
                 <td style="text-align:center"><?php echo (date('Y-m-d H-i-s',$vo["time"])); ?></td>
                   ...
                   <td style="text-align:center"><?php echo ($vo["beizhu"]); ?></td>
                    <td style="text-align:center"><?php if($vo["statue"] == '1'): ?><span  style="color:red">√</span><?php else: ?>×<?php endif; ?></td>
                 ...
                 <!-- <td><?php echo ($vo["click"]); ?></td>-->
				  ...
				  <!-- <a href="<?php echo U('Wap/Index/content',array('token'=>$_SESSION['token'],'id'=>$vo['id']));?>">查看</a> -->
				   ...
				   <a href="<?php echo U('Adcms/hezuo_do',array('id'=>$vo['id'],'token'=>$vo['token']));?>" >标记受理</a>

				   <a href="<?php echo U('Adcms/hezuodel',array('id'=>$vo['id'],'token'=>$vo['token']));?>">删除</a></td>
