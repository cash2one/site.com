...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
  ...
  <td><?php echo ($list["count"]); ?>条</td>
  <td>
  	<a href="<?php echo U('Wall/check_msg',array('token'=>$token,'id'=>$pid,'uid'=>$list['id']));?>">查看信息</a>
  	|
  	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Wall/del_fens',array('token'=>$token,'id'=>$list['id'],'pid'=>$pid));?>');">删除</a>
  </td>
...
<script>
	function showShakeRank(id,pid){
    art.dialog.open('<?php echo U('Shake/show_rank',array('token'=>$token));?>&id='+id+'&pid='+pid,{lock:false,title:'信息详情',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.87});
	}
</script>
 </div>
