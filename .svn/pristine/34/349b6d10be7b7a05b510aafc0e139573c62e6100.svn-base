...
// 				$save_file2 = BASE_UPLOAD_PATH.'/'.$_POST['filename'];
				$save_file2 = str_ireplace(UPLOAD_SITE_URL,BASE_UPLOAD_PATH,$_POST['filename']);
                                $save_path =  str_ireplace(BASE_UPLOAD_PATH,"",$save_file2);
...
//
//				//上传图片
//				$upload->save2($save_file2);
//			}
                        
                        //将图片上传到OSS
                        import('libraries.upload.oss');
                        $upload_oss = new oss();  
                        $file_type = mime_content_type($save_file2);  //读取文件类型
                        
                        $rs =  $upload_oss->save($save_file2,$file_type,$save_path);
