...
class goodsControl extends BaseGoodsControl {
     ...
     * 单个商品信息页
	 * 更新商品评论信息
	 * update by lizh 12:14 2016/7/22
     */
    public function indexOp() {
		...
		$store_data = Model('store') -> field('store_id,store_name,store_avatar, store_banner') -> where(array(store_id => $goods_info['store_id'])) -> find();
		...
		$all_evaluate_info = Model('evaluate_goods')-> where(array(geval_goodsid => $goods_id, geval_state => 0, up_geval_id => 0)) -> select();

		foreach($all_evaluate_info as $k => $v) {
			
			$geval_id = $v['geval_id'];
			$child_evaluate_info = Model('evaluate_goods')-> where(array(geval_goodsid => $goods_id, geval_state => 0, up_geval_id => $geval_id)) -> select();
			$all_evaluate_info[$k]['child_evaluate_info'] = $child_evaluate_info;
				
			
			
		}
		
		Tpl::output('all_evaluate_info', $all_evaluate_info);
        ...
        //Tpl::showpage('goods');
        Tpl::showpage('xiangqingye');
    }
	
	/**
	 * ajax商品评论
	  * update by lizh 14:56 2016/6/30
	 */
	public function  memberEvaluateInfoOp() {
		
		$insertArray['geval_content'] = $_POST['geval_content'];
		$insertArray['geval_frommemberid'] = $_POST['geval_frommemberid'];
		$insertArray['geval_frommembername'] = $_POST['geval_frommembername'];
		$insertArray['geval_goodsid'] = $_POST['geval_goodsid'];
		$insertArray['geval_goodsimage'] = $_POST['geval_goodsimage'];
		$insertArray['geval_goodsname'] = $_POST['geval_goodsname'];
		$insertArray['geval_goodsprice'] = $_POST['geval_goodsprice'];
		$insertArray['geval_isanonymous'] = $_POST['geval_isanonymous'];
		$insertArray['geval_ordergoodsid'] = $_POST['geval_ordergoodsid'];
		$insertArray['geval_orderno'] = $_POST['geval_orderno'];
		$insertArray['geval_scores'] = $_POST['geval_scores'];
		$insertArray['geval_state'] = $_POST['geval_state'];
		$insertArray['geval_storeid'] = $_POST['geval_storeid'];
		$insertArray['geval_storename'] = $_POST['geval_storename'];
		 $insertArray['up_geval_id'] = $_POST['up_geval_id'];
		$insertArray['geval_addtime'] = time();
		
		$result = Model() -> table('evaluate_goods') -> insert($insertArray);
		echo json_encode(array(status => 1, info => $result), JSON_UNESCAPED_UNICODE);
		
	}
	
	/**
	 * ajax用户评论点赞
	 * add by lizh 17:49 2016/6/28
	 */
	public function  user_approveOp() {
		
		$geval_id = $_GET['geval_id'];
		$evaluate_goods = Model('evaluate_goods');
		$evaluate_goods_data = $evaluate_goods -> field('geval_approve') -> where(array(geval_id => $geval_id)) -> find();
		$geval_approve = $evaluate_goods_data['geval_approve'];
		$geval_approve += 1;
		
		$result = Model() -> table('evaluate_goods') -> where(array(geval_id => $geval_id)) -> update(array(geval_approve => $geval_approve));
		echo json_encode(array(status => 1, info => $result), JSON_UNESCAPED_UNICODE);
	 ...
	 * add by lizh 17:49 2016/6/28
	 */
	public function getDownGoodsClassOp() {
		
		$gc_id = $_GET['gc_id'];
		$down_goods_class_array = Model('goods_class') -> where(array(gc_parent_id => $gc_id )) -> select();
		if(empty($down_goods_class_array)) {
			
			$info = 0;
			
		} else {
			
			$info = $down_goods_class_array;
			foreach($info as $k => $v) {
				
				$a_href = C('shop_site_url') . '/index.php?act=search&op=index&cate_id=' . $v['gc_id'];
				$info[$k]['a_href'] = $a_href;
			}
			
			
		}
		
		echo json_encode(array(status => 1, info => $info), JSON_UNESCAPED_UNICODE);
