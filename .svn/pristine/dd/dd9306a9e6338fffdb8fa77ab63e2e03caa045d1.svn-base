...
//            Tpl::showpage('evaluation.add');
            ...
            foreach ($order_goods as $value){
                //如果未评分，默认为5分
                $evaluate_score = intval($_GET['goods'][$value['goods_id']]['score']);
                if($evaluate_score <= 0 || $evaluate_score > 5) {
                    $evaluate_score = 5;
                }
                //默认评语
                $evaluate_comment = $_GET['goods'][$value['goods_id']]['comment'];
                ...
                $evaluate_goods_info['geval_isanonymous'] = $_GET['anony']?1:0;
            ...
            $store_desccredit = intval($_GET['store_desccredit']);
            if($store_desccredit <= 0 || $store_desccredit > 5) {
                $store_desccredit= 5;
            }
            $store_servicecredit = intval($_GET['store_servicecredit']);
            if($store_servicecredit <= 0 || $store_servicecredit > 5) {
                $store_servicecredit = 5;
            }
            $store_deliverycredit = intval($_GET['store_deliverycredit']);
...
//             //添加店铺评价
    ...
    public function add_image_saveOp() {
        $geval_id = intval($_GET['geval_id']);
        $geval_image = '';
        foreach ($_GET['evaluate_image'] as $value) {
            if(!empty($value)) {
                $geval_image .= $value . ',';
            }
        }
