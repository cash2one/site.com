...
class store_goods_addControl extends BaseSellerControl {
     ...
     * 添加商品
     */
    public function add_step_twoOp() {//http://www.wanyi123.com/shop/index.php?act=store_goods_add&op=add_step_two&class_id=1542&t_id=0
        // 实例化商品分类模型
        $model_goodsclass = Model('goods_class');
        ...
        if (!isset($data[$gc_id]) || isset($data[$gc_id]['child']) || isset($data[$gc_id]['childchild'])) {
            showDialog(L('store_goods_index_again_choose_category1'));//您选择的分类不存在，或没有选择到最后一级，请重新选择分类。
        }
        ...
        if (!checkPlatformStoreBindingAllGoodsClass()) {
            ...
            //echo '<pre>';print_r($bind_info);exit;
            if (empty($bind_info))
            {
                $where['class_3'] =  0;
                $bind_info = $model_bind_class->getStoreBindClassInfo($where);
                if (empty($bind_info))
                {
                    ...
                    if (empty($bind_info))
                    {
                        ...
                        if (empty($bind_info))
                        {
                            ...
                            showDialog(L('store_goods_index_again_choose_category2'));//您的店铺没有绑定该分类，请重新选择分类。
                        }
                    }
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_alias']        = $_POST['goods_alias'];//新增商品别名
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_alias']       = $common_array['goods_alias'];//新增商品别名
