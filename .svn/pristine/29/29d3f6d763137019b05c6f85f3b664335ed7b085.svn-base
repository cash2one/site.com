...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
 ...
 <div id="showdiv<?php echo ($vo["imicms_id"]); ?>">
   ...
   <div id="testdiv<?php echo ($vo["imicms_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["imicms_id"]); ?>&modle=Cutprice" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($vo["imicms_id"]); ?>" style="width:149px;"><center>查看二维码</center></a><br><?php endif; endif; ?>
							<a href="<?php echo U('Cutprice/order',array('token'=>$token,'id'=>$vo['imicms_id']));?>">订单</a>
							<a href="<?php echo U('Cutprice/update',array('token'=>$token,'id'=>$vo['imicms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Cutprice/operate',array('token'=>$token,'id'=>$vo['imicms_id'],'type'=>'del'));?>')">删除</a>
						</td>
