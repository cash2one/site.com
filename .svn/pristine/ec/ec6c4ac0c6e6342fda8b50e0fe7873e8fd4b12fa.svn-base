		...
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($bargain_list)): $i = 0; $__LIST__ = $bargain_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<?php if($vo["state"] == 1): ?><td><a href="javascript:state(<?php echo ($vo["imicms_id"]); ?>)"><img id="state<?php echo ($vo["imicms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/bargain/images/start.png" width="100%"/></a></td>
						<?php else: ?>
						<td><a href="javascript:state(<?php echo ($vo["imicms_id"]); ?>)"><img id="state<?php echo ($vo["imicms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/bargain/images/stop.png" width="100%"/></a></td><?php endif; ?>
						<td class="norightborder" style="border-right:1px solid #eee">
							<script type="text/javascript">
   $(document).ready(function(){
      var bheight=document.body.clientHeight;
      $("#lay<?php echo ($vo["imicms_id"]); ?>").click(function(){
   $("#brg<?php echo ($vo["imicms_id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($vo["imicms_id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($vo["imicms_id"]); ?>").click(function(){
      $("#brg<?php echo ($vo["imicms_id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($vo["imicms_id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($vo["imicms_id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($vo["imicms_id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute;top:20%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($vo["imicms_id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($vo["imicms_id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($vo["imicms_id"]); ?>"></div>
 <div id="showdiv<?php echo ($vo["imicms_id"]); ?>">
   <div id="close<?php echo ($vo["imicms_id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($vo["imicms_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["imicms_id"]); ?>&modle=Bargain" width="300" height="300" class="cateimg_small"></div>
 </div>
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><a id="lay<?php echo ($vo["imicms_id"]); ?>" style="width:150px;">查看二维码</a><br><?php endif; endif; ?>
							<a href="<?php echo U('Bargain/order',array('token'=>$token,'id'=>$vo['imicms_id']));?>">订单</a>
							<a href="<?php echo U('Bargain/update',array('token'=>$token,'id'=>$vo['imicms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Bargain/operate',array('token'=>$token,'id'=>$vo['imicms_id'],'type'=>'del'));?>')">删除</a>
						</td>
