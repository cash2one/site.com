	...
	public function article_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				//update by lizh 10:11 2016/7/8
				$upload = new UploadFile();
				$upload->set('default_dir',ATTACH_CMS.'/ac_icon');
				if(!empty($_FILES['article_wap_icon'])) {
					
					$result	= $upload->upfile('article_wap_icon');
					if(!empty($upload->file_name)) {
						
						$insert_array['article_wap_icon'] = $upload->file_name;
			...
			if ($error != ''){
				...
				if(!empty($_FILES['article_wap_icon'])) {
					...
					if(!empty($upload->file_name)) {
						...
						$update_array['article_wap_icon'] = $upload->file_name;
