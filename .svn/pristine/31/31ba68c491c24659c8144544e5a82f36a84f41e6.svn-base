	...
	public function store_settingOp(){
		...
		if (chksubmit()){
			 ...
			 */
				...
				$result = $upload->upfile('store_avatar');
				if ($result){
			...
			if (!empty($_FILES['app_banner']['name'])){
				...
				$upload->set('file_name','');
				$upload->set('ifremove',false);
				$result = $upload->upfile('app_banner');
				if ($result){
					$_POST['app_banner'] = $upload->file_name;
				}else {
					showDialog($upload->error);
				}
			}
			...
			//删除旧店铺图片app_banner
			if (!empty($_POST['app_banner']) && !empty($store_info['app_banner'])){
			    @unlink(BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$store_info['app_banner']);
			}
			/**
			 * 更新入库
			 */
            $param = array(
                ...
                'app_banner' => empty($_POST['app_banner']) ? $store_info['app_banner'] : $_POST['app_banner'],
