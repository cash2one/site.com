...
class storeControl extends SystemControl{
	...
	public function storeOp(){
		...
		if(trim($_GET['owner_and_name']) != ''){
			$condition['seller_name']	= array('like', '%'.$_GET['owner_and_name'].'%');
        ...
        if(intval($_GET['sc_id']) > 0){
            ...
            Tpl::output('sc_id',intval($_GET['sc_id']));
        }
        if(is_numeric($_GET['is_recommend'])){
            $condition['is_recommend']		= intval($_GET['is_recommend']);
            Tpl::output('is_recommend',intval($_GET['is_recommend']));
        }
        if(is_numeric($_GET['is_brand'])){
            $condition['is_brand']		= intval($_GET['is_brand']);
            Tpl::output('is_brand',intval($_GET['is_brand']));
        }
        if(is_numeric($_GET['is_artisan'])){
            $condition['is_artisan']		= intval($_GET['is_artisan']);
            Tpl::output('is_artisan',intval($_GET['is_artisan']));
        }

        switch ($_GET['store_state']) {
            case 'close':
        ...
        Tpl::output('store_state', $this->_get_store_state_array());
    ...
    public function ajaxOp(){
        ...
        switch ($_GET['branch']){
            ...
            case 'is_recommend':
                if(intval($_GET['id']) > 0) {
                    $model= Model('store');
                    $condition['store_id'] = intval($_GET['id']);
                    $update[$_GET['column']] = trim($_GET['value']);
                    $model->editStore($update,$condition);
                    echo 'true';die;
                } else {
                    echo 'false';die;
                }
	...
	public function store_editOp(){
		...
		if (chksubmit()){
			...
			$update_array['salesperson'] = $_POST['salesperson'];
