...
class buy_1Logic {
    ...
    public function pdPay($order_list, $input, $buyer_info) {
     ...
     /**
     * 壹积金支付,依次循环每个订单
     * 如果壹积金足够就单独支付了该订单，如果不足就暂时冻结，等API支付成功了再彻底扣除
     */
    public function onempf_pay($order_list, $input, $buyer_info) {
        $member_id = $buyer_info['member_id'];
        $member_name = $buyer_info['member_name'];

        $available_onempf_amount = floatval($buyer_info['member_onempf']);
        if ($available_onempf_amount <= 0) $order_list;
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_amount = floatval($order_info['order_amount']) - floatval($order_info['onempf_amount']);
            $data_onempf = array();
            $data_onempf['member_id'] = $member_id;
            $data_onempf['member_name'] = $member_name;
            $data_onempf['amount'] = $order_amount;
            $data_onempf['order_sn'] = $order_info['order_sn'];

            if ($available_onempf_amount >= $order_amount) {
                //壹积金立即支付，订单支付完成
                $model_onempf->changeOnempf('order_pay',$data_onempf);
                $available_onempf_amount -= $order_amount;

                //支付被冻结的充值卡(预定订单没有冻结，是直接扣除，除外)
                $onempf_amount = floatval($order_info['onempf_amount']);
                if ($onempf_amount > 0 && $order_info['order_type'] != 2) {
                    $data_pd = array();
                    $data_pd['member_id'] = $member_id;
                    $data_pd['member_name'] = $member_name;
                    $data_pd['amount'] = $onempf_amount;
                    $data_pd['order_sn'] = $order_info['order_sn'];
                    $model_onempf->changeOnempf('order_comb_pay',$data_pd);
                }
                ...
                $order_list[$key]['payment_time'] = $data_order['payment_time'] = TIMESTAMP;
                $order_list[$key]['payment_code'] = $data_order['payment_code'] = 'predeposit';
                $order_list[$key]['onephf_amount'] = $data_order['onephf_amount'] = $order_amount;
...
//                 output_data($result);
                ...
                //暂冻结预存款,后面还需要 API彻底完成支付
                if ($available_onempf_amount > 0) {
                    $data_pd['amount'] = $available_onempf_amount;
                    $model_onempf->changeOnempf('order_freeze',$data_pd);
                    //预存款支付金额保存到订单
                    $data_order = array();
                    $order_list[$key]['onephf_amount'] = $data_order['onephf_amount'] = $available_onempf_amount;
                    $result = $model_order->editOrder($data_order,array('order_id'=>$order_info['order_id']));
                    $available_onempf_amount = 0;
                    if (!$result) {
                        throw new Exception('订单更新失败');
                    }
                }
	...
	/**
	 * 生成礼品卡支付单编号(两位随机 + 从2000-01-01 00:00:00 到现在的秒数+微秒+会员ID%1000)，该值会传给第三方支付接口
	 * 长度 =2位 + 10位 + 3位 + 3位  = 18位
	 * 1000个会员同一微秒提订单，重复机率为1/100
	 * @return string
	 * add by lizh 16:54 2016/8/3
	 */
	public function cardMakePaySn($member_id) {
		return 'c'.mt_rand(10,99)
		      . sprintf('%010d',time() - 946656000)
		      . sprintf('%03d', (float) microtime() * 1000)
		      . sprintf('%03d', (int) $member_id % 1000);
	}
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['is_firstcut'] = $goods_info['is_firstcut'];
        $new_array['is_fcode'] = $goods_info['is_fcode'];
        $new_array['is_happysend'] = $goods_common_info['is_happysend'];
    ...
    public function extendInPay($order_list) {
        ...
        if ($order_list[0]['order_type'] == 2) {
            ...
            if (!empty($order_info['onempf_amount'])) {
                $data['book_onempf_amount'] = $order_info['onempf_amount'];
            }

            //如果未使用站内余额，返回
            if (empty($order_info['rcb_amount']) && empty($order_info['pd_amount'])&&empty($order_info['onempf_amount'])) {
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['is_firstcut'] = $goods_online_info['is_firstcut'];
