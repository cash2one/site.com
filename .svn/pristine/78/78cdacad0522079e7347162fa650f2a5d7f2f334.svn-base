...
$(function(){
	...
	$('.nav_select_div li').live('click',function(){
		...
		$.getJSON("<?php echo U('Web/get_nav_list');?>",{type:type},function(result){
			if(result.error == 1){
				alert(result.msg);
...
.nav_select_div .nav_content a{display:block;margin-top:5px;background:#EBEEF1;color: #464D6A;padding:0.5em;cursor:pointer;text-decoration:none;}
 ...
 <div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4>编辑导航</h4><a href="<?php echo U('Web/nav');?>"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div> 
<div class="msgWrap">
  <form class="form" method="post" name="addform" action="<?php echo U('Web/nav',array('act'=>'nav_add_amend'));?>" enctype="multipart/form-data" >
	<input type="hidden" name="id" value="<?php echo ($now_nav["id"]); ?>"/>
<TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
  <THEAD>
<TR>
  <TH valign="top"><label for="name">导航名称：</label></TH>
  <TD><input type="text" class="px" id="name" value="<?php echo ($now_nav["name"]); ?>" name="name" style="width:300px" /><br />
                  </TD>
  <TD>&nbsp;</TD>
</TR>
                            </THEAD>
  ...
  <TD><input type="text" class="px" id="s_name" value="<?php echo ($now_nav["s_name"]); ?>" name="s_name" style="width:300px" /> &nbsp;大多都是模板中设定导航名称的英文词语<br />
                  </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
	...
	<TD>
		<select name="fid">
			...
			<?php if(is_array($pc_nav)): $i = 0; $__LIST__ = $pc_nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($now_nav['fid'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
    </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  ...
  <TD><input type="text" class="px" id="url" value="<?php echo ($now_nav["url"]); ?>" name="url" style="width:300px" />&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" id="nav_select" class="a_upload" style="color:red;">根据模型选择链接</a>
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="key">关键词：</label></TH>
  <TD><input type="text" class="px" id="key" value="<?php echo ($now_nav["key"]); ?>" name="key" style="width:300px" /><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="sort">排序：</label></TH>
  <TD><input type="text" class="px" id="sort" value="<?php echo ($now_nav["sort"]); ?>" name="sort" style="width:100px" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;数值越大，排序越前。（默认添加越早，排序越前）<br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
