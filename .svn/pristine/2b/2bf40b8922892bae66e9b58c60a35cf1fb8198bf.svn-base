...
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('CoinTree/add_action',array('token'=>$token));?>" title="添加摇钱树活动" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 添加活动</a> </div>
<div class="searchbar right">
<form method="post" action="<?php echo U('CoinTree/index',array('token'=>$token));?>">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
 ...
 <div id="showdiv<?php echo ($list["id"]); ?>">
   ...
   <div id="testdiv<?php echo ($list["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($list["id"]); ?>&modle=CoinTree" width="300" height="300" class="cateimg_small"></div>
 </div>
	<a id="lay<?php echo ($list["id"]); ?>"  style="width:191px;">查看二维码</a><br><?php endif; endif; ?>
		<a href="<?php echo U('CoinTree/prizerecord',array('token'=>$list['token'],'id'=>$list['id']));?>">摇奖记录</a>
		|
		<a href="<?php echo U('CoinTree/add_action',array('token'=>$list['token'],'id'=>$list['id']));?>">编辑</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('CoinTree/del_action',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
