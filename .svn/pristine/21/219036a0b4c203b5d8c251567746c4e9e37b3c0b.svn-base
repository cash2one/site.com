...
$(document).ready(function(){
    ...
    $('span[nc_type="like_count"]').inline_edit({act: 'microshop',op: 'personal_like_count_update'});
    //时间
...
<div class="page">
    ...
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
          ...
          <th class="w108">瞬间话题</th>
          <th><?php echo $lang['microshop_commend_message'];?></th>
          <th class="w96 align-center">点赞数量</th>
      ...
      <tbody>
        ...
        <tr class="hover edit">
		  ...
		  <td>
			<select onchange="setclass_list(this.value,<?php echo $value['personal_id']; ?>)">
				<?php if($value['class_id'] == 0){?>
					<option value ="<?php echo $value['class_id'];?>" selected='selected'>无话题</option>
				<?php } else { ?>
					<option value ="<?php echo $value['class_id'];?>">无话题</option>
				<?php } ?>
				<?php foreach($output['class_list'] as $key2 => $value2) { ?>
					<option value ="<?php echo $value2['class_id'];?>" <?php if($value['class_id'] == $value2['class_id']){ ?>selected='selected' <?php } ?>><?php echo $value2['class_name'];?></option>
				<?php } ?>
          ...
          <td><?php echo $value['commend_message'];?></td>
           <td class="w48 sort"><span nc_type="like_count" column_id="<?php echo $value['personal_id'];?>" title="<?php echo $lang['nc_editable'];?>" class="editable "><?php echo $value['like_count'];?></span></td>
  ...
  <script>
	...
	function setclass_list(class_value,id) {
		var url = <?php echo "'".ADMIN_SITE_URL."'";?>+'/index.php?act=microshop&op=setclass_list';
		$.ajax({
			
			type:'GET',
			url:url,
			data:{'class_id':class_value,'personal_id':id},
			dataType:'json',
