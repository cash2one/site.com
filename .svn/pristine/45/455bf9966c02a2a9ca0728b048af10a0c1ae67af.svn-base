...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            //注册时间
            $member_time = $model_member->table('member')->getfby_member_id($member_id, 'member_time');
            ...
            //是否过期，超过两个月 byniro
            $if_overtime =  61-floor(($order_info['finnshed_time'] - $member_time) / 86400 );
            if ($if_overtime > 0) {
                $rebate_amount_onempf = 0.01 * $order_info['order_amount'] * $GLOBALS['setting_config']['onempf_rebate'];
            }
            if($rebate_amount_onempf){
            Model('onempf')->saveOnempfLog('rebate', array('member_id' => $inviter_id, 'member_name' => $inviter_name, 'rebate_amount' => $rebate_amount_onempf), true);
            }
            //分享的返佣
            $share_id =  $model_order->table('order')->getfby_order_id($order_id, 'share_id');
            if($share_id){
                 Model('onempf')->saveOnempfLog('rebate', array('member_id' => $share_id, 'member_name' => $inviter_name, 'rebate_amount' => $rebate_amount_onempf), true);
            }
            return callback(true, '操作成功');
        } catch (Exception $e) {
            return callback(false, '操作失败');
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array()) {
        ...
        try {
            ...
            $data['e_code'] = $post['e_code'];
            $content = Model('express')->get_express($data['e_code'], $data['shipping_code']);
        ...
        //友盟推送
        $rs = set_umeng_push($param['member_id'], '订单发货提醒', '你购买的商品已经发货', array(type => 5,id => $order_id));
