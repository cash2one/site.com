...
<div class="cLine">
<a href="<?php echo U('FrontPage/index',array('token'=>$token));?>" class="right btnGrayS" style="margin-top:-1px;height:24px;">返回活动列表</a>
<div class="searchbar right">
<form method="post" action="<?php echo U('FrontPage/fansnews',array('token'=>$token,'news_type'=>2,'id'=>$_GET['id']));?>">
<input type="text" id="msgSearchInput" class="txt left" placeholder="请输入新闻标题搜索" name="search_word" value="<?php echo ($search_word); ?>" style="padding:6px;width:155px;">
...
<?php if($list != ''): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>  
	<td><input type="checkbox" name="news_id[]" value="<?php echo ($list['id']); ?>"></td>
	<td><?php echo ($list["wecha_name"]); ?></td>
	<td><?php echo ($list["news_title"]); ?></td>
	<td title="<?php echo ($list["news_txt"]); ?>"><?php echo (mb_substr($list["news_txt"],0,50,'utf-8')); ?></td>
	<td><?php if($list['create_time'] != ''): echo (date('Y-m-d H:i:s',$list["create_time"])); else: ?>---<?php endif; ?></td>
	<td><a href="<?php echo ($list["voicepath"]); ?>" target="_blank"><?php echo ($list["voicepath"]); ?></a></td>
	<td class="norightborder" align="center">
	<a href="javascript:drop_confirm('您确定要删除该事件吗?', '<?php echo U('FrontPage/deldefinenews',array('token'=>$token,'id'=>$list['id'],'news_type'=>2,'cid'=>$_GET['id']));?>');">删除</a>
    </td>
</tr><?php endforeach; endif; else: echo "" ;endif; ?>
<?php else: ?>
<tr><td colspan="8">暂无记录</td></tr><?php endif; ?>
</tbody>
</table>
<?php if($list != ''): ?><div style="float:left;">  <input type="checkbox" id="checkall" /><label for="checkall"> 全选 / 全不选  <button class="btnGrayS" id="batch_del">批量删除事件</button></div><?php endif; ?>
</div>
<div class="cLine">
