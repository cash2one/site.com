...
<div class="cLine">
<div class="pageNavigator left"> <a href="###" onclick="$('#info').submit()" title="批量审核" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/arrow_switch.png" class="vm"><?php endif; ?> 批量审核</a></div>
<div class="clr"></div>
</div>
...
<tr></tr>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
			<?php  $photos_array = explode(',',$t['photos']); if($photos_array[0] != ''){ foreach($photos_array as $pk=>$pv){ echo "<a class='fancybox' href='".$pv."'>查看图片".($pk+1)."</a><br/>"; } }else{ echo "无"; } ?>
			</td>
