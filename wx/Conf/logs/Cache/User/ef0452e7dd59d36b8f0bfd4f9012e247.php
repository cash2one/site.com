...
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('FrontPage/addconfigure',array('token'=>$token));?>" title="配置信息" class="btnGrayS vm bigbtn">添加配置</a> </div>
<a href="<?php echo U('FrontPage/index',array('token'=>$token));?>" class="right btnGrayS" style="margin-top:-1px;height:24px;">返回活动列表</a>
<div class="searchbar right">
<form method="post" action="<?php echo U('FrontPage/listconfigure',array('token'=>$token));?>">
<input type="text" id="msgSearchInput" class="txt left" placeholder="请输入apikey搜索" name="search_word" value="<?php echo ($search_word); ?>" style="padding:6px;width:155px;">
...
<?php if($list != ''): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	<td><?php echo ($list["apikey"]); ?></td>
	<td><?php echo ($list["secretkey"]); ?></td>
	...
	<td class="norightborder" align="center">
		<?php if(time() - $list['addtime'] > $list['expires_in']){ ?><a href="<?php echo U('FrontPage/updateconfigure',array('token'=>$list['token'],'id'=>$list['id']));?>">更新令牌</a>
		| <?php } ?>
		<a href="<?php echo U('FrontPage/addconfigure',array('token'=>$list['token'],'id'=>$list['id']));?>">编辑</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('FrontPage/delconfigure',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
