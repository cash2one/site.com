...
<div class="content">
	<div class="cLineB">
	 ...
	 <div class="cLineB">
		...
		<a href="<?php echo U('SiteMessage/index', array('token'=>$token, 'id'=>$data['id'], 'status'=>1));?>" class="btnGrayS vm <?php if (!empty($_GET['status'])):?>navHover<?php endif?>" style="float:right;background-color:#B6B8B4;">已读</a>
        <a href="<?php echo U('SiteMessage/index', array('token'=>$token, 'id'=>$data['id']));?>" class="btnGrayS vm <?php if (empty($_GET['status'])):?>navHover<?php endif?>" style="float:right;background-color:#B6B8B4;">未读</a>
...
<tbody>
  	<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class): $mod = ($i % 2 );++$i;?><tr class="hover">
		<td><input type="checkbox" value="<?php echo ($class["id"]); ?>" class="cbitem" name="id[]"></td>
        <td class="td25">
			<span><?php echo ($i); ?></span>
		</td>
        <td>
			<span><?php echo ($class["from"]); ?></span>
        </td>
        <td>
        ...
        <td>
        	<a href="<?php echo U('SiteMessage/status',array('id'=>$class['id']));?>" class="ajax"><?php echo $class['status'] ? '<span style="color:green;">已读</span>' : '<span style="color:red;">未读</span>' ?></a>
        </td>
        <td>
        	<?php echo (date("Y-m-d H:i:s",$class["create_time"])); ?>
        </td>
        <td>
			<a class=" btnGreen dialog" href="<?php echo U('SiteMessage/view',array('id'=>$class['id']));?>">查看</a>
			<a class=" btnGreen " href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('SiteMessage/del',array('id'=>$class['id'], 'status'=>$_GET['status']));?>');">删除</a>
		</td>				
