...
<?php else: ?>
	<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;font-size:12px;"><?php endif; ?>
		温馨提示：本功能必须开启网页授权并选择获取昵称头像等所有信息！<br/>
	</div>
	<div class="cLine">
		<div class="pageNavigator left"> <a href="<?php echo U('Sentiment/set',array('token'=>$token));?>" title="添加活动" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 添加活动</a></div>
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
 ...
 <style type="text/css">
   ...
   #showdiv<?php echo ($vo["id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute; width:100px; height:100px; top:20%; left:50%; margin:-50px 0 0 -50px;}
 ...
 <div id="showdiv<?php echo ($vo["id"]); ?>">
   ...
   <div id="testdiv<?php echo ($vo["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["id"]); ?>&modle=Sentiment" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($vo["id"]); ?>"  style="width:191px;">查看二维码</a><br><?php endif; endif; ?>
