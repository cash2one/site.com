...
function setNewsImage(type){
	...
	// 此时 iframeA.html 页面可以使用 art.dialog.data('test') 获取到数据，如：
	// document.getElementById('aInput').value = art.dialog.data('test');
	art.dialog.open('<?php echo U('Diymen/imagenews',array('token'=>$token));?>&type='+type,{lock:false,title:'选择'+text,width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
...
<div class="content">
    ...
    <?php else: ?>
    <div class="ftip" style="margin:10px auto;"><?php endif; ?>
	...
	<div id="czselarea">	
		...
		<div class="control-group alert alert-info">
			  <label class="control-label" for="answertype">菜单类型:</label>
			    <div class="controls">			    
					<select name="menu_type" class="menu_type" id="answertype">
						<option value="1" <?php if($type == 1): ?>selected<?php endif; ?>>图文</option>
						<option value="2" <?php if($type == 2): ?>selected<?php endif; ?>>图片</option>
					</select>
			   </div>
		 </div>		 
			<!-- 图文  -->
			<div class="szcjbt alert alert-info" <?php if($show["keyword"] != ''): ?>style="display:block;"<?php endif; ?> id="res_1">
				选择图文：<input type="text" name="keyword" readonly="readonly" id="menu_keyword" value="<?php echo ($show["keyword"]); ?>" >
			...
			<div class="szcjbt alert alert-info"<?php if($show["url"] != ''): ?>style="display:block;"<?php endif; ?> id="res_2">
				选择图片：<input type="text" name="url" readonly="readonly" id="menu_key" value="<?php echo ($show["url"]); ?>" >
...
<tr>
   ...
   <tbody>
  	<?php if(is_array($class)): $i = 0; $__LIST__ = $class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class): $mod = ($i % 2 );++$i;?><tr class="hover">
        ...
        <td>
        	<span>
	        	    ...
	        	    <?php if($class["keyword"] != ''): echo D('Img')->isOpenSync($wxuser) ? '顶级菜单-【图文菜单】' : '顶级菜单-【关键词回复菜单】'; ?>
	  ...
	  <?php if(is_array($class['class'])): $i = 0; $__LIST__ = $class['class'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class1): $mod = ($i % 2 );++$i;?><tr class="td29">
			...
			<td colspan="1">
				<span>
					<?php if($class1["keyword"] != ''): echo D('Img')->isOpenSync($wxuser) ? '图文菜单' : '关键词回复菜单'; ?>		        		
