  ...
  <div class="ncs-main">
    <div class="flexslider">
      <ul class="slides">
        ...
        <li><a <?php if($output['store_slide_url'][$i] != '' && $output['store_slide_url'][$i] != 'http://'){?>href="<?php echo $output['store_slide_url'][$i];?>"<?php }?>><img src="<?php echo sliderImage($output['store_slide'][$i]);?>"></a></li>
        <?php }?>
        <?php }?>
        <?php }else{?>
            ...
            <dl>
              <dt><a href="<?php echo urlShop('goods', 'index', array('goods_id'=>$value['goods_id']));?>" class="goods-thumb" target="_blank"><img src="<?php echo thumb($value, 240);?>" alt="<?php echo $value['goods_name'];?>"/></a>
                ...
                <?php if (is_array($value['image'])) {?>
                  ...
                  <li<?php if($i==1) {?> class="selected"<?php }?>><a href="javascript:void(0);"><img src="<?php echo thumb($val, 60);?>" data-img="<?php echo thumb($val, 240);?>" /></a></li>
                  <?php }?>
                <?php } else {?>
                  <li class="selected"><a href="javascript:void(0)"><img src="<?php echo thumb($value, 60);?>" data-img="<?php echo thumb($value, 240);?>"></a></li>
                <?php }?>
                </ul>
              </dt>
...
<script type="text/javascript">
	$(window).load(function() {
	    ...
	    $('.goods-thumb-scroll-show').find('a').mouseover(function(){
          ...
          var _src = $(this).find('img').data('img');
