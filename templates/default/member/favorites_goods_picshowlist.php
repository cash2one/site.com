    ...
    <thead>
      <tr nc_type="table_header">
       <div style="margin-top: 5px;float: left">
            ...
            <a href="javascript:void(0);" class="ncm-btn-mini" uri="index.php?act=member_favorites&op=delfavorites&type=goods" name="fav_id" confirm="<?php echo $lang['nc_ensure_del'];?>" nc_type="batchbutton">
                <i class="icon-trash"></i><?php echo $lang['nc_del'];?>
            </a>
            </div>
            <div class="model-switch-btn" style="width: 65%">
                <p style="float:left;margin-top: 5px">收藏列表：</p>
                <a style="width: 10%;margin-top: 5px;<?php if($_GET['favorites_class_id'] == null || $_GET['favorites_class_id'] == 0){ ?>color:red<?php } ?>" href="index.php?act=member_favorites&op=fglist" title="默认">默认</a>
                <?php if(!empty($output['favorites_class_list'])){ ?>
                <?php foreach($output['favorites_class_list'] as $kfcl => $vfcl) { ?>
                <a style="width: 10%;margin-top: 5px;<?php if($_GET['favorites_class_id'] == $vfcl['favorites_class_id']){ ?>color:red<?php } ?>" href="index.php?act=member_favorites&op=fglist&favorites_class_id=<?php echo $vfcl['favorites_class_id']; ?>" title="<?php echo $vfcl['favorites_class_name']; ?>"><?php if(mb_strlen($vfcl['favorites_class_name'],'utf-8') > 5){echo mb_substr($vfcl['favorites_class_name'],0,5,'utf-8').'..';} else { echo $vfcl['favorites_class_name'];} ?></a>
                <?php } ?>
                <?php } ?>
            </div>
            <div class="model-switch-btn" style="margin-top: 5px;float:right">
                <?php echo $lang['favorite_view_mode'].$lang['nc_colon'] ;?>
                <a href="index.php?act=member_favorites&op=fglist&show=list" title="<?php echo $lang['favorite_view_mode_list'];?>"><i class="icon-list"></i></a>
                <a href="index.php?act=member_favorites&op=fglist&show=pic" class="current" title="<?php echo $lang['favorite_view_mode_pic'];?>"><i class="icon-picture"></i></a>
                <a href="index.php?act=member_favorites&op=fglist&show=store" title="<?php echo $lang['favorite_view_mode_shop'];?>"><i class="icon-home"></i></a>
            </div>
      </tr>
    </thead>
    <tbody>
