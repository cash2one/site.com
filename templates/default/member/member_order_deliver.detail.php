...
$(function(){
	...
	$.getJSON('index.php?act=member_order&op=get_express&order_id=<?php echo $output['order_info']['order_id']?>',function(data){
		if(data){
                   $('.loading').hide();
                   $.each(data.express_inf, function (n, value) {
               var trs = "";
               trs +=  "<li>" + value.time + value.context + "</li>";
                 console.log(trs);
               $('#express_list').append(trs);
           }); 
		}else{
